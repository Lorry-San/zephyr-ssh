...
function toolDefinitions(ai = {}) {
    ...
    tools.push({ type: 'function', function: { name: 'remote_desktop_screenshot', description: '实时读取用户当前 Zephyr RDP/VNC 远程桌面最新画面快照（JPEG 截图）。RDP/VNC 没有文本终端输出；当你需要确认刚才操作是否生效、页面是否加载完成、当前画面是什么时调用本工具。调用后前端会立即从当前 iframe/canvas 重新截屏，不复用旧上下文截图。', parameters: { type: 'object', properties: { tabId: { type: 'string' }, maxWidth: { type: 'number' } } } } });
